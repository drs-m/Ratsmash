...
(function() {
  $(function() {
    return $("#description_described_name").autocomplete({
      messages: {
        result: function(count) {},
        noResults: ''
      },
      source: function(request, response) {
        return $.get("/vote/autocomplete.json?q=" + request.term, function(data) {
          var result, suggestions, _i, _len, _ref;
          suggestions = [];
...
(function() {
  $(function() {
    var category, showErrorMsg, showSuccessMsg;
    ...
    $(".candidate-input").autocomplete({
      ...
      source: function(request, response) {
        return $.get("/vote/autocomplete.json?q=" + request.term + "&c=" + category, function(data) {
          var result, suggestions, _i, _len, _ref;
          suggestions = [];
          if (data.status === "success") {
            _ref = data.results;
    ...
    showSuccessMsg = function(form, msg) {
      ...
      return $(form).find(".success-msg").fadeOut().text(msg).fadeIn();
    };
    return showErrorMsg = function(form, msg) {
      $(form).find(".success-msg").fadeOut();
      return $(form).find(".error-msg").fadeOut().text(msg).fadeIn();
    };
  });
    ...
    return $("#login-form").submit(function(event) {
      ...
      return $.post("/login.json", {
        ...
        if (data.status === "error") {
          ...
          $("#error-msg").fadeIn();
          return $("#login-form").effect("shake");
...
function add_poll_voting_opportunity(){
	...
	var new_poll_option_field = '<b>Votingmöglichkeit</b> <input id="voting_op_'+(poll_option_fields_counter-1)+'" name="voting_op['+(poll_option_fields_counter-1)+']" value="" onkeyup="store_poll_option_values('+(poll_option_fields_counter-1)+');" type="text" /><br /><br />';
